  ...
  return this.each(function() {
    ...
    scrollForward = function() {
      ...
      var curLi,
          ...
          scrollDistance,
          theStep;
      var newlength=containerUL.children("li").length;
      ...
      if (newScrollOffset >= scrollDistance) {
        for (i = 0; i < config.switchItems; i++) {
          if (config.queue && config.queue.hasChildNodes() && config.queue.getElementsByTagName('li').length > 0) {
            ...
            //containerUL.remove(containerUL.children('li').eq(0));
            //containerUL.append(containerUL.children('li:first-child'));
          }
        }
        if(newlength>39&&nowMoveIndex+39<=newlength)
        {
          alert(containerUL.children('li').length);
